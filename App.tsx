...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => { if (isAdminLoggedIn) fetchData(); }, [isAdminLoggedIn]);
  ...
  const handleSubmit = async (e: React.FormEvent, shouldClear: boolean = false) => {
    ...
    if (error) {
      setAdminStatus({ type: 'error', msg: 'Error: ' + error.message });
    } else {
      setAdminStatus({ type: 'success', msg: 'Data Synced Successfully!' });
      if (shouldClear) {
      ...
      <div className="max-w-7xl mx-auto w-full space-y-8">
        <header className="flex justify-between items-center bg-white/5 p-8 rounded-[2.5rem] border border-white/10 backdrop-blur-md">
          <div className="text-right">
            ...
            <p className="text-cyan-400 text-xs font-bold tracking-widest uppercase">Central Administrator Panel</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <aside className="lg:col-span-3 bg-white/5 p-6 rounded-[2rem] border border-white/10">
            <h3 className="orbitron text-[10px] text-cyan-400 mb-6 tracking-widest font-black uppercase">Users Portal</h3>
            <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              <button onClick={() => setActiveAdminEmail(null)} className={`w-full text-right p-4 rounded-2xl transition-all ${activeAdminEmail === null ? 'bg-cyan-600 text-white' : 'hover:bg-white/5 text-gray-500'}`}>General Questions</button>
              {userList.map(ema => (
          ...
          <main className="lg:col-span-9 space-y-8">
            <section className="bg-white/5 p-10 rounded-[2.5rem] border border-white/10">
              <h2 className="text-2xl font-black mb-8">Managing: <span className="text-cyan-400">{activeAdminEmail || 'Public Sector'}</span></h2>
              <form onSubmit={e => handleSubmit(e)} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  <textarea required placeholder="Enter Question Text..." className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-lg font-bold outline-none text-white focus:border-cyan-500 h-32" value={adminFormData.question_text} onChange={e => setAdminFormData({...adminFormData, question_text: e.target.value})} />
                </div>
            ...
            <section className="bg-white/5 p-10 rounded-[2.5rem] border border-white/10">
              <h3 className="orbitron text-center text-gray-500 mb-8 font-black uppercase">Data Nodes Log</h3>
              <div className="space-y-4">
                {allQuestions.filter(q => q.assigned_to_email === activeAdminEmail).map(q => (
                  <div key={q.id} className="bg-black/40 p-6 rounded-[2rem] flex justify-between items-center border border-white/5">
                    ...
                    <button onClick={async () => { if(confirm('Delete?')) { await supabase.from('custom_questions').delete().eq('id', q.id); fetchData(); } }} className="mr-6 p-4 bg-red-500/10 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all">DELETE</button>
                  </div>
                ))}
              </div>
            </section>
...
const App: React.FC = () => {
  ...
  const syncQuestions = async (target: string | null) => {
    try {
      if (target) {
        const { data } = await supabase.from('custom_questions').select('*').eq('assigned_to_email', target);
    ...
    } catch { setActiveLevels(HARDCODED_LEVELS); }
  };
  ...
  return (
    <div className="relative w-full h-screen bg-[#050510] text-white flex flex-col overflow-hidden text-right" dir="rtl">
        ...
        <div className="relative z-20 h-full w-full flex flex-col items-center justify-center p-6 text-center animate-fade-in pointer-events-auto">
          ...
          <div className="flex flex-col md:flex-row gap-6 w-full max-w-2xl">
            <button 
              ...
              className="flex-1 py-5 px-10 bg-white/5 border border-white/10 rounded-2xl text-xl font-bold hover:bg-white/10 transition-all active:scale-95 shadow-lg"
            >
              <span className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">ابدأ الآن</span> النسخة المجانية
            </button>
            <button 
              onClick={() => setShowAuthModal(true)} 
              className="flex-1 py-5 px-10 bg-gradient-to-r from-cyan-600 to-blue-700 rounded-2xl text-xl font-black hover:from-cyan-500 hover:to-blue-600 transition-all active:scale-95 shadow-[0_0_30px_rgba(8,145,178,0.5)]"
            >
              <span className="block text-[10px] text-cyan-200 uppercase tracking-widest mb-1">الاحترافية</span> النسخة المدفوعة
            </button>
          </div>
      ...
      {gameState === GameState.BRIEFING && (
        <MissionBriefing level={levelIndex + 1} question={activeLevels[levelIndex].question} onEngage={() => setGameState(GameState.PLAYING)} />
      )}
        ...
        <div className="relative w-full h-full flex flex-col z-10 pointer-events-auto">
          ...
          <div className="flex-1 w-full relative">
            <GameView 
              ...
              onCorrect={() => { 
                setScore(s => s + 500); setLastFeedback({ type: 'success', message: 'ACCESS GRANTED' }); 
                setTimeout(() => { 
                  setLastFeedback(null); 
                  if (levelIndex < activeLevels.length - 1) { setLevelIndex(i => i + 1); setGameState(GameState.BRIEFING); }
              ...
              onEnemyHit={() => {
                ...
                setLastFeedback({ type: 'fail', message: 'INTEGRITY FAIL' }); setTimeout(() => setLastFeedback(null), 1500);
              }} 
      ...
      {gameState === GameState.PRO_SUCCESS && (
        <div className="z-30 h-full w-full flex flex-col items-center justify-center bg-[#050510] text-center p-10 animate-fade-in">
          <div className="mb-10 text-emerald-400">
             <svg className="w-24 h-24 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          </div>
